...
export RUNNAME=${FREERUNEXP}
export FCSTRUNNAME=${FREERUNEXP}
export FCSTFIELD="cntlBkg"
export MODELNAME="FreeRun-cntlBkg"
export OBSRUNNAME=${FREERUNEXP}
export OBSFIELD="cntlBkg"
export OBSNAME="FreeRun-cntlBkg"
...
do
    ...
    cd $WRKD
    /bin/sh $subcmd -a $PROJ_ACCOUNT -p $POPOTS -j $METRUN-${FCSTVAR}_${OBSVAR} -o ${WRKD}/$METRUN-${FCSTVAR}-${OBSVAR}.out -q batch -t 03:29:00 -r /1 ${RUNSCRIPT}
    #sleep 2
...
cd $WRKD
##rm -rf $WRKD/*
/bin/sh $subcmd -a $PROJ_ACCOUNT -p $POPOTS -j $METRUN-${FCSTVAR}-${OBSVAR} -o ${WRKD}/$METRUN-${FCSTVAR}-${OBSVAR}.out -q urgent -t 00:29:00 -r /1 ${RUNSCRIPT}
exit 0
